...
export excel using "$out_folder/tabla_balance_matching.xlsx", firstrow(variables) replace


* C) Saco un control iterando, para tener 6. Solo saco controles con peso 0.5

	* Potenciales controles a eliminar
use `base', clear

levelsof ID_institucion if _treated==0 & _weight==0.5, local(controles_05)
display "`controles_05'"
...
foreach c of local controles_05 {
...
export excel using "$out_folder/tabla_balance_matching_eliminando.xlsx", firstrow(variables) replace

use `base', clear

/*
	Opción 2: Matching manual
*/
